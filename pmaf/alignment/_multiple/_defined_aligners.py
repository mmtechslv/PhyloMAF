...
def clustalw2_wrapper(in_tmp_file, out_tmp_file, param_dict={}, cmd_bin='clustalw2'):
    ...
    if len(param_dict)>0:
        clustal_exec = ClustalwCommandline(cmb_bin_adj, infile=in_tmp_file.name,outfile=out_tmp_file.name,**param_dict)
    else:
        clustal_exec = ClustalwCommandline(cmb_bin_adj, infile=in_tmp_file.name, outfile=out_tmp_file.name)
