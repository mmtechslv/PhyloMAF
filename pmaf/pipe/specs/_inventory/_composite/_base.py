from pmaf.pipe.specs._metakit import (
    SpecificationCompositeMetabase,
    SpecificationBackboneMetabase,
)
...
class SpecificationCompositeBase(SpecificationBase, SpecificationCompositeMetabase):
    ...
    def __init__(self, _specs, _steps):
        if not all(
            [isinstance(spec, SpecificationBackboneMetabase) for spec in _specs]
        ):
