...
class SpecTSPBP(SpecificationCompositeBase):
    ...
    def __define_lazy_steps(self):
        steps_dict = [('identifier-to-phylogeny-sequence-to-phylogeny', self.__checkpoint_identifier_to_branched_phylogeny, DockerPhylogenyMedium,'Retrieve phylogeny and sequences from database by identifiers. Followed by branch estimation to produce branched phylogeny..')]
        return steps_dict
    ...
    def __checkpoint_identifier_to_branched_phylogeny(self,docker,*args,**kwargs):
        ...
        new_metadata = {'master': {phylogeny_docker.__class__.__name__:phylogeny_docker.wrap_meta(),alignment_docker.__class__.__name__:alignment_docker.wrap_meta()}}
        branched_phylogeny_docker = DockerPhylogenyMedium(tmp_branched_phylos,name=docker.name,metadata=new_metadata)
