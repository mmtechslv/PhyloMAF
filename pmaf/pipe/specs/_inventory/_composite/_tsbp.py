...
class SpecTSBP(SpecificationCompositeBase):
    ...
    def __define_lazy_steps(self):
        steps_dict = [
            (
                ...
                self.__checkpoint_sequence_to_branched_phylogeny,
                DockerPhylogenyMedium,
                "Retrieve aligned sequences from database by identifiers. Followed by de-novo building phylogenetic tree.",
            )
        ]
        return steps_dict
    ...
    def __checkpoint_sequence_to_branched_phylogeny(self, docker, *args, **kwargs):
        ...
        branched_phylogeny_docker = DockerPhylogenyMedium(
            [builded_tree], name=docker.name, metadata=new_metadata
        )
