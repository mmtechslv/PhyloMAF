...
class SpecTI(SpecificationPrimitiveBase):
    ...
    def __define_lazy_steps(self):
        steps_dict = [
            ...
            (
                "taxonomy-to-identifier",
                self.__checkpoint_taxonomy_to_identifier,
                DockerIdentifierMedium,
                "Retrieve identifiers by taxonomy.",
            ),
        ]
        return steps_dict
    ...
    def verify_docker(self, docker):
        return self.miner.verify_docker(docker) and isinstance(
            docker, DockerTaxonomyMetabase
        )
