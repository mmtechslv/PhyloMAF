...
class MediatorLocalBase(MediatorBase):
    ...
    def verify_factor(self, factor):
        ...
        if isinstance(factor,FactorBackboneMetabase):
            ...
            gene_name = (factor.factors.get('gene-name', None) in self.client.summary['gene']) or self.client.summary['gene'] == 'N/A'
            gene_target = (factor.factors.get('gene-target', None) in self.client.summary['gene-target-region']) or self.client.summary['gene-target-region'] == 'N/A'
