...
class MediatorLocalTaxonomyMixin(MediatorLocalBase,MediatorTaxonomyMetabase):
    ...
    def __init__(self,database,
                 ...
                 tax_corr_method='lineage',
                 tax_fuzzy_cutoff=95,
                 tax_fuzzy_mode=False,
                 tax_format = None,
                 tax_refrep='tid',
                 **kwargs):
        ...
        if isinstance(tax_fuzzy_cutoff, int):
            if not (tax_fuzzy_cutoff > 0 and tax_fuzzy_cutoff < 100):
                raise ValueError('`tax_corr_method` must be greater than 0 and less than 100. ')
            if tax_fuzzy_cutoff < 90:
                warnings.warn("Warning! Assigned cutoff value is too low. Consider value greater than 90.")
        else:
    ...
    def __taxon_correlator(self, ref_taxa_series, target_taxa_array):
        tmp_product = pd.DataFrame(index=target_taxa_array, columns=['match', 'method', 'score'], data=0, dtype=object)
        
        exact_match_mask = ref_taxa_series.isin(target_taxa_array)
        exact_match_map = ref_taxa_series[exact_match_mask].reset_index().set_index(ref_taxa_series.name)['index']
        tmp_product.loc[exact_match_map.index, 'match'] = exact_match_map.values
        tmp_product.loc[exact_match_map.index, 'method'] = 'exact'
        
        uncorrelated_taxa = target_taxa_array[~np.isin(target_taxa_array, ref_taxa_series.values)]
        uncorrelated_taxa_map = pd.Series(data=uncorrelated_taxa, index=uncorrelated_taxa)
        ...
        if len(uncorrelated_taxa)>0 and len(remaining_ref_taxa)>0:
            ...
            remaining_ref_taxa_rev = remaining_ref_taxa.reset_index().set_index(remaining_ref_taxa.name)['index']
            fuzzy_correlated_map = uncorrelated_taxa_map.map(lambda taxon: process.extractOne(taxon, remaining_ref_taxa_flat, scorer=fuzz.ratio,score_cutoff=self.configs['tax_fuzzy_cutoff']))
            tmp_product.loc[fuzzy_correlated_map.index, 'match'] = fuzzy_correlated_map.map(lambda corr: remaining_ref_taxa_rev[corr[0]] if corr is not None else 0)
            tmp_product.loc[fuzzy_correlated_map.index, 'method'] = 'fuzzy'
            tmp_product.loc[fuzzy_correlated_map.index, 'score'] = fuzzy_correlated_map.map(lambda corr: corr[1] if corr is not None else None)
    ...
    def __correlator_taxon_by_taxonomy(self,docker,*kwargs):
        ...
        matching_df = pd.DataFrame(data=0, index=docker.index, columns=shared_ranks_sorted, dtype=int)
        match_method_df = pd.DataFrame(data='', index=docker.index, columns=shared_ranks_sorted, dtype=str)
        match_score_df = pd.DataFrame(data=0, index=docker.index, columns=shared_ranks_sorted, dtype=int)

        matched_tids = None
        last_rank = None
        for rank in shared_ranks_sorted:
            target_rank_taxonomy = docker.to_dataframe(ranks=rank, exclude_missing=True).dropna()
            ...
            if matched_tids is None:
                ...
                target_rank_groups = target_rank_gpy.apply(lambda tax_group: tax_group.index.values)
                target_rank_taxonomy_unique = target_rank_groups.index.values
                matched_map = self.__taxon_correlator(ref_rank_taxonomy_filtered, target_rank_taxonomy_unique)
                ...
                tid_subs_map = self.client.find_sub_tids_by_tid(matched_tids, ter_rank=rank, flatten=False, mode='dict')
                target_rank_tid_gpy = matching_df.groupby(last_rank)
                for prior_tid, target_taxon_ids in target_rank_tid_gpy.groups.items():
                    if prior_tid != 0:
                        target_rank_sub_taxonomy = target_rank_taxonomy.loc[target_rank_taxonomy.index.isin(target_taxon_ids)]
                        potential_rank_ref_tids = tid_subs_map[prior_tid]
                        ref_rank_taxonomy_filtered = ref_rank_taxonomy.loc[potential_rank_ref_tids].drop_duplicates()
                        target_rank_sub_taxon_gpy = target_rank_sub_taxonomy.groupby(target_rank_sub_taxonomy)
                        target_rank_sub_groups = target_rank_sub_taxon_gpy.apply(lambda tax_group: tax_group.index.values)
                        target_rank_sub_taxonomy_unique = target_rank_sub_groups.index.values
                        matched_map = self.__taxon_correlator(ref_rank_taxonomy_filtered, target_rank_sub_taxonomy_unique)
                        for taxon, tid, method, score in matched_map.itertuples():
                            ...
                            match_score_df.loc[target_taxon_ids, rank] = score
                matched_tids = matching_df.loc[matching_df.loc[:, rank] != 0, rank].values
            last_rank = rank
        ...
        valid_taxa_map_df = pd.DataFrame(index=target_taxonomy.index, columns=shared_ranks_sorted, data=False, dtype=bool)

        for rev_rank in shared_ranks_sorted[::-1]:
            valid_terminal_taxa = ((target_taxonomy[rev_rank].notna()) & (matching_df[rev_rank] != 0))
            valid_non_terminal_taxa = ((target_taxonomy[rev_rank].isna()) & (matching_df[rev_rank] == 0))
            valid_taxa_map_df.loc[:, rev_rank] = (valid_terminal_taxa | valid_non_terminal_taxa)

        valid_taxa = valid_taxa_map_df.loc[matching_df.index].all(1)

        target_ref_map = matching_df.loc[valid_taxa].apply(lambda row: int(row[row != 0].values[-1]) if sum(row == 0) != len(row) else 0, axis=1, result_type='reduce').astype(int)
        correlations_meta = pd.DataFrame(index=target_ref_map.index, columns=['matches', 'method'], dtype=object)
        correlations_meta.loc[:, 'matches'] = target_ref_map.values
        correlations_meta.loc[:, 'method'] = match_method_df.loc[target_ref_map.index, shared_ranks_sorted[-1]]
        uncorrelated_taxa = target_taxonomy.index[~target_taxonomy.index.isin(target_ref_map.index)]

        rank_trials['kickoff'] = {'correlations': correlations_meta.to_dict(orient='index'),
        ...
        if len(rank_tolerance) > 0 and len(uncorrelated_taxa) > 0:
            for limiting_rank in [rank for rank in MAIN_RANKS[::-1] if rank in rank_tolerance.keys()]:
                tolerated_matching_df = matching_df.loc[uncorrelated_taxa, matching_df.columns[~matching_df.columns.isin(rank_tolerance[limiting_rank])]]
                tolerated_shared_ranks_sorted = sort_ranks(tolerated_matching_df.columns.tolist())

                tolerated_target_ref_map = tolerated_matching_df.loc[uncorrelated_taxa].apply(lambda row: int(row[row != 0].values[-1]) if sum(row == 0) != len(row) else 0, axis=1, result_type='reduce').astype(int)
                tolerated_correlations_meta = pd.DataFrame(index=tolerated_target_ref_map.index, columns=['matches', 'method'], dtype=object)
                tolerated_correlations_meta.loc[:, 'matches'] = tolerated_target_ref_map.values
                tolerated_correlations_meta.loc[:, 'method'] = match_method_df.loc[tolerated_target_ref_map.index, tolerated_shared_ranks_sorted[-1]]
                target_ref_map = target_ref_map.append(tolerated_target_ref_map)
                uncorrelated_taxa = target_taxonomy.index[~target_taxonomy.index.isin(target_ref_map.index)]

                rank_trials[limiting_rank] = {'correlations': tolerated_correlations_meta.to_dict(orient='index'),
                                              'uncorrelated': uncorrelated_taxa.values.tolist(),
                ...
                if target_ref_map[ix] != 0:
                    ...
                    for rank in shared_ranks_sorted:
                        if matching_df.loc[ix, rank] != 0:
                            tmp_ix_rank_details = {'reference': ref_taxonomy.loc[matching_df.loc[ix, rank], rank], 'target': target_taxonomy.loc[ix, rank]}
                            if match_method_df.loc[ix, rank] == 'exact':
                                tmp_ix_rank_metadata[rank] = {'matching-method': 'exact', 'match': matching_df.loc[ix, rank], 'matching-details': tmp_ix_rank_details}
                            else:
                                tmp_ix_rank_metadata[rank] = {'matching-method': 'fuzzy', 'matching-score': match_score_df.loc[ix, rank], 'matching-details': tmp_ix_rank_details}
        ...
        new_metadata = {'configs': self.configs,'rank-trials':dict(rank_trials),'verbose': dict(identifiers_metadata), 'master': docker.wrap_meta()}
        return DockerIdentifierMedium(identifiers=identifier_dict, name=docker.name, metadata=new_metadata)
    ...
    def __complement_taxonomy_by_taxonomy(self, docker, **kwargs):
        ...
        shared_avail_ranks = [rank for rank in docker_avail_ranks if rank in db_avail_ranks]
        ...
        while len(shared_avail_ranks_tolerated) > 0 and (not total_match):
            database_lineages = generate_lineages_from_taxa(tdb_taxonomy_low.loc[:, db_avail_ranks], True, shared_avail_ranks_tolerated)
            target_lineages = generate_lineages_from_taxa(target_taxonomy_low.loc[index_unmatched, :], True, shared_avail_ranks_tolerated)
            correlations = self.__primary_lineage_correlator(target_lineages, database_lineages)
            ...
            for ix, corrs, method in matched_corr.itertuples():
                if method == 'exact':
                    ...
                    tmp_matches_metadata = {'win': selected_corr, 'all': corrs, 'lineages': {corr: database_lineages[corr] for corr in corrs}}
                else:
                    tmp_corrs = sorted(corrs, key=lambda fzc: fzc[1], reverse=True)
                    selected_corr = tmp_corrs[0][0]
                    tmp_matches_metadata = {'win': selected_corr, 'all': tmp_corrs, 'lineages': {corr: database_lineages[corr] for corr, _ in tmp_corrs}}
                tmp_correlations[ix] = selected_corr
                tmp_corr_metadata[ix] = {'method': method, 'verbose': tmp_matches_metadata, 'rank-tolerance-trial': trial}
                tmp_pass_ranks = get_rank_upto(docker_avail_ranks[::-1],shared_avail_ranks_tolerated[-1])
                tmp_transit[ix] = {'method':method,'lost-taxonomy':{rank:docker.data[ix][rank] for rank in tmp_pass_ranks}}
            tmp_metadata_ranks[trial] = {'ranks': tuple(shared_avail_ranks_tolerated), 'matched': len(matched_corr), 'unmatched': len(index_unmatched)}
            shared_avail_ranks_tolerated = shared_avail_ranks_tolerated[:-1]
            total_match = True if len(index_unmatched) == 0 else False
            trial = trial + 1
        new_metadata = {'configs':self.configs,'correlations':tmp_corr_metadata,'master': docker.wrap_meta()}
        return DockerIdentifierMedium(identifiers=tmp_correlations, name=docker.name, metadata=new_metadata,_transit=tmp_transit)
    ...
    def __retrieve_taxonomy_by_identifier(self, docker, **kwargs):
        ...
        if self.configs['tax_refrep'] == 'tid':
            ...
            tmp_taxonomy_df = self.client.get_taxonomy_by_rid(ids=id_array, tax_format=self.configs['tax_format'])
        else:
            raise ValueError('`tax_refrep` is invalid.')
        ...
        if self.configs['tax_corr_method'] == 'complement' and docker._transit is not None:
        ...
        return DockerTaxonomyMedium(taxonomy=tmp_taxonomy,name=docker.name,metadata=new_metadata)
    ...
    def __correlator_lineage_by_taxonomy(self, docker, **kwargs):
        ...
        ref_lineages = generate_lineages_from_taxa(ref_taxonomy, missing_rank=True, desired_ranks=shared_ranks)
        target_taxonomy =  docker.to_dataframe(exclude_missing=True,ranks=shared_ranks)
        correlations, matching_details, rank_trials = self.__correlate_dfs_by_lineage(ref_lineages, target_taxonomy, shared_ranks, parsed_rank_tolerance)
        return self.__parse_lineage_correlation_result(docker, correlations, matching_details, rank_trials)

    def __parse_lineage_correlation_result(self, master_taxonomy_singleton, correlations, matching_details, rank_trials):
            ...
            if ix in correlations.index:
                ...
                for match_result in tmp_correlation['matches']:
                    if tmp_correlation['method'] == 'exact':
                        ...
                        identifiers_metadata[ix] = {'matching-method':'exact','matching-details':{'reference':matching_details['ref-values'][match_result],'target':matching_details['target-values'][ix]}}
                    else:
                        identifiers_dict[ix] = match_result[0]
                        identifiers_metadata[ix] = {'matching-method': 'fuzzy','matching-score': match_result[1],'matching-details':{'reference':matching_details['ref-values'][match_result[0]],'target':matching_details['target-values'][ix]}}
            else:
                identifiers_dict[ix] = None
                identifiers_metadata[ix] = {'matching-method': 'fuzzy'}
        new_metadata = {'configs':self.configs,'rank-trials':rank_trials,'verbose':dict(identifiers_metadata),'master':master_taxonomy_singleton.wrap_meta()}
        return DockerIdentifierMedium(identifiers=identifiers_dict,name=master_taxonomy_singleton.name,metadata=new_metadata)

    def __correlate_dfs_by_lineage(self, ref_lineages, target_taxonomy, shared_ranks, rank_tolerance):
        self_lineages = generate_lineages_from_taxa(target_taxonomy,True, shared_ranks,False)
        correlations = self.__primary_lineage_correlator(self_lineages, ref_lineages)
        uncorrelated_feature_ids = self_lineages.index[~self_lineages.index.isin(correlations.index)]
        uncorrelated_lineages = self_lineages[uncorrelated_feature_ids]
        rank_trials = defaultdict(dict)
        matching_details = {'target-values':self_lineages[correlations.index].to_dict()}
        rank_trials['kickoff'] = {'correlations': correlations.to_dict(orient='index'),
        ...
        if len(rank_tolerance)>0 and len(uncorrelated_lineages)>0:
            for limiting_rank in [rank for rank in MAIN_RANKS[::-1] if rank in rank_tolerance.keys()]:
                self_lineages = generate_lineages_from_taxa(target_taxonomy,True, shared_ranks, rank_tolerance[limiting_rank])
                unassigned_self_lineages = self_lineages.loc[uncorrelated_feature_ids]
                new_correlations = self.__primary_lineage_correlator(unassigned_self_lineages, ref_lineages)
                correlations = correlations.append(new_correlations)
                uncorrelated_feature_ids = self_lineages.index[~self_lineages.index.isin(correlations.index)]
                uncorrelated_lineages = self_lineages[uncorrelated_feature_ids]
                matching_details['target-values'].update(self_lineages[new_correlations.index].to_dict())
                rank_trials[limiting_rank] = {'correlations': new_correlations.to_dict(orient='index'),
                                              'uncorrelated': uncorrelated_feature_ids.values.tolist(),
                                              'added': len(new_correlations),
                                              'tolerated-ranks': rank_tolerance[limiting_rank]}
                if len(uncorrelated_lineages)==0:
                    break
        matched_ids = [list(match_elem) for match_elem in correlations[correlations['method']=='exact']['matches'].values]
        matched_ids.extend([list(fuzzy_elem[0] for fuzzy_elem in match_elem) for match_elem in correlations[correlations['method']=='fuzzy']['matches'].values])
        matched_ids_flat = [matched_id for matched_ids_elem in matched_ids for matched_id in matched_ids_elem]
        matching_details.update({'ref-values':ref_lineages[matched_ids_flat].to_dict()})
        return correlations, matching_details, rank_trials

    def __primary_lineage_correlator(self, primary_lineages, secondary_lineages):
        ...
        Args:
            primary_lineages (pandas.Series): pandas Series of target taxonomy with
                taxon IDs as index and lineage as values
            secondary_lineages (pandas.Series): pandas Series of reference taxonomy
                with taxon IDs as index and lineage as values

        Returns:
            pd.Dataframe: Dictionary with correlation results. Result format:
        ...
        gr_ref_lin_df = matchlin_gby.apply(lambda lin_group: lin_group.index.values).reset_index()
        ...
        correlated_lineages = trg_lin_df.join(gr_ref_lin_df, how='inner').set_index('target')
        correlated_lineages['method'] = 'exact'
        if self.configs['tax_fuzzy_mode']:
            uncorrelated = target_lineages[~target_lineages.index.isin(correlated_lineages.index)]
            ...
            remaining_ref_lin_map = remaining_ref_lin.reset_index().set_index('lineage')['index'].to_dict()


            fuzzy_correlated_lineages = uncorrelated.map(lambda lin: process.extractOne(lin, remaining_ref_lin_flat, scorer=fuzz.ratio, score_cutoff=self.configs['tax_fuzzy_cutoff']))
            fuzzy_correlated_lineages_fixed = fuzzy_correlated_lineages.map(lambda corr: [(remaining_ref_lin_map[corr[0]],corr[1])] if corr is not None else []).to_frame('matches')
            fuzzy_correlated_lineages_fixed['method'] = 'fuzzy'
            correlated_lineages = correlated_lineages.append(fuzzy_correlated_lineages_fixed)
        correlated_lineages_valid = correlated_lineages[correlated_lineages['matches'].map(lambda tmp_matches:len(tmp_matches)).astype(bool)]
        return correlated_lineages_valid


    # Here rank lists are generated until it reaches upto_rank. For example, if upto_rank='f' and shared_avail_ranks=['c', 'o', 'f', 'g', 's'] then return will contain {'f': ['g', 's'], 'g': ['s']}
    def __parse_rank_tolerance(self, target_ranks):
        target_ranks_sorted = sort_ranks(target_ranks)
        if (self.configs['tax_rank_tolerance'] in target_ranks_sorted) and ((target_ranks_sorted.index(self.configs['tax_rank_tolerance']) + 1) != len(target_ranks_sorted)):
            tol_index = len(target_ranks_sorted) - target_ranks_sorted.index(self.configs['tax_rank_tolerance'])
            return {target_ranks_sorted[-(e + 1)]: target_ranks_sorted[-e:] for e in range(1, tol_index)}
        else:
            return {}