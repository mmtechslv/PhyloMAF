...
from pmaf.sequence._metakit import SequenceBackboneMetabase,MultiSequenceMetabase,NucleotideMetabase
...
class DockerSequenceMedium(DockerSequenceMetabase,DockerBase):
    ...
    def __init__(self, sequences, mode='DNA', aligned=None, **kwargs):
        ...
        if isinstance(sequences, self._UNIT_TYPES):
            ...
            for seq_id, seq_str in seqio.pull_parser(id=True, sequence=True):
                if validate_seq_grammar(seq_str, type):
                    ...
                    raise ValueError('`sequences` file contain sequences with invalid grammar.')
        ...
        if all([isinstance(sequence,(SequenceBackboneMetabase,type(None))) for sequence in tmp_sequences.values()]):
            tmp_sequences_valid = {seq_id: seqs for seq_id, seqs in tmp_sequences.items() if seqs is not None}
            if len(tmp_sequences_valid)>0:
                tmp_seq_modes_test = set([sequence.mode for sequence in tmp_sequences_valid.values()]+([mode_fixed] if mode_fixed is not None else []))
                if len(tmp_seq_modes_test)!=1:
                    raise ValueError('`sequences` and/or specified mode does not match.')
                if mode_fixed is None:
                    mode_fixed = tmp_seq_modes_test.pop()
                if all([isinstance(sequence,MultiSequenceMetabase) for sequence in tmp_sequences_valid.values()]):
                    tmp_seq_algn_test = set([sequence.is_alignment for sequence in tmp_sequences_valid.values()] + ([aligned] if aligned is not None else []))
                    if aligned is not None:
                        tmp_aligned = bool(aligned)
                        if len(tmp_seq_algn_test)!=1:
                            raise ValueError('`sequences` and/or specified alignment states does not match')
                    else:
                    ...
                    for seq_id, seqs in tmp_sequences_valid.items():
                        tmp_sequences_adj[seq_id] = type(self)(seqs,mode=seqs.mode,aligned=tmp_aligned,name=seqs.name,metadata=seqs.metadata)
                elif all([isinstance(sequence,NucleotideMetabase) for sequence in tmp_sequences_valid.values()]):
                    if aligned:
                        if len(set([sequence.length for sequence in tmp_sequences_valid.values()]))==1:
                            tmp_aligned = True
                        else:
                            raise ValueError('`sequences` must have same length if aligned.')
                    else:
                        tmp_aligned = aligned
                    tmp_sequences_adj = dict.fromkeys(tmp_sequences.keys(),None)
                    tmp_metadata_new = {'master-metadata':dict.fromkeys(tmp_sequences_valid.keys())}
                    for seq_id, seqs in tmp_sequences_valid.items():
        ...
        container_mode_test = any([isinstance(data_elem, type(self)) for data_elem in tmp_sequences_adj.values()])
        if container_mode_test:
            ...
            for ix, tax_medium in tmp_sequences_adj.items():
                if isinstance(tax_medium,type(self)):
                    ...
                    tmp_mode_test.add(tax_medium.mode.lower() if tax_medium.mode is not None else None)
            ...
            if len(tmp_aligned_test)>1 or len(tmp_mode_test)>1:
                raise ValueError('`sequences` must have same attributes for container usage.')
        ...
        super().__init__(_data_dict=tmp_sequences_adj, _valid_types=self._UNIT_TYPES, metadata=tmp_metadata, **kwargs)
        ...
        if self.singleton:
            ...
            for ix in target_indices:
                if self.data[ix] is not None:
                    tmp_sequences.append(Nucleotide(self.data[ix],name=ix,mode=self.__mode,metadata={'taxid': ix}))
            return MultiSequence(tmp_sequences,name=self.name,metadata=self.metadata,internal_id='taxid',aligned=self.__aligned)
        else:
    ...
    def get_records(self, indices=None,exclude_missing=False):
        ...
        if self.singleton:
            ...
            for ix in target_indices:
                if self.data[ix] is not None:
                    tmp_records_dict[ix] = self.data[ix].get_records(exclude_missing=exclude_missing)
    ...
    def get_stats(self, indices=None, exclude_missing=False):
        ...
        if self.singleton:
            ...
            for ix in target_indices:
                if self.data[ix] is not None:
                    tmp_records.append(make_sequence_record_tuple(ix, self.data[ix]))
            tmp_records_df = pd.DataFrame.from_records(tmp_records, columns=['rid', 'sequence', 'length', 'tab'], index=['rid'])
            return get_stats_for_sequence_record_df(tmp_records_df)
        else:
            tmp_stats_dict = dict.fromkeys(target_indices,None)
            for ix in target_indices:
                if self.data[ix] is not None:
                    tmp_stats_dict[ix] = self.data[ix].get_stats(exclude_missing=exclude_missing)
            return next(iter(tmp_stats_dict.values())) if np.isscalar(indices) else tmp_stats_dict
