    ...
    def __init__(self, accessions, **kwargs):
        if isinstance(accessions, list):
            try:
                ...
                raise ValueError('`accessions` provided as list must have following format [(ACS_NAME:ACS_NO),(ACS_NAME:ACS_NO),...].')
        elif isinstance(accessions, pd.Series):
            tmp_accessions = {kwargs.get('name', 0): {accessions.name:accessions.to_dict()}}
        elif isinstance(accessions,pd.DataFrame):
            tmp_accessions = accessions.to_dict(orient='index')
        elif isinstance(accessions, dict):
            if all([isinstance(value, (int, str, np.integer, tuple, type(None))) for value in accessions.values()]):
                tmp_accessions = {kwargs.get('name', 0): accessions}
            else:
                tmp_accessions = accessions
        else:
            tmp_accessions = {kwargs.get('name', 0): accessions}
        container_mode_test = any([isinstance(data_elem, type(self)) for data_elem in tmp_accessions.values()])
            ...
            for ix, accs_dict in tmp_accessions.items():
                if accs_dict is not None:
                    for src, _ in accs_dict.items():
                        tmp_sources.add(src)
                    tmp_accessions_adj[ix] = None if all([taxa is None for taxa in accs_dict.values()]) else accs_dict
                else:
                ...
                if self.data[ix] is not None:
                    if source in self.data[ix].keys():
                        if isinstance(self.data[ix][source],tuple)>0:
                            tmp_identifiers[ix] = DockerIdentifierMedium(self.data[ix][source],name=ix,metadata={'source':source})
                        else:
            ...
            for ix in target_indices:
                if self.data[ix] is not None:
                    if source in self.data[ix].sources:
                        tmp_identifiers[ix] = self.data[ix].to_identifier_by_src(source, exclude_missing)
        new_metadata = {'source':source,'master':self.wrap_meta()}
        return DockerIdentifierMedium(tmp_identifiers,name=self.name,metadata=new_metadata)
