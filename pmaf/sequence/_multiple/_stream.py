...
from pmaf.sequence._metakit import MultiSequenceMetabase,MultiSequenceStreamBackboneMetabase
...
class MultiSequenceStream(MultiSequenceStreamBackboneMetabase):
    _temp_filename_suffix_maker = lambda self, path_with_preffix: '{}_pmaf_{}'.format(path_with_preffix,str(round(100000000*random())))
    ...
    def __init__(self,filepath=None,expected_rows=1000, mode='DNA', aligned=False, name=None,compressor=False):
        self._name = ''
        self._mode = None
        ...
        if isinstance(compressor,str):
            if compressor in self._supported_compression_libraries:
                ...
                raise ValueError('Compressor is not supported. Please use one of {}'.format(','.join(self._supported_compression_libraries)))
        ...
        if isinstance(filepath,str):
            ...
            tmp_temp_filepath = self._temp_filename_suffix_maker(os.path.join(tempfile.gettempdir(), tempfile.gettempprefix()))
    ...
    def __repr__(self):
        ...
        repr_str = "<{}: {}, Total Sequences: {}, Filepath: {}, Aligned: {}>".format(class_name, name, count, stream_filepath, aligned)
        return repr_str
    ...
    def _init_seq_stream_storer(self):
        ret = False
        try:
            tmp_filters = tables.Filters(complib=self._compressor, complevel=self._default_complevel) if self._compressor else None
            tmp_stream_store = tables.open_file(self._stream_filepath, mode='a', title=self._name, filters=tmp_filters)
            tmp_stream_store.create_group("/", 'seq', 'Raw sequences')
            tmp_stream_store.create_vlarray('/seq', 'seqs', atom=tables.VLStringAtom(), expectedrows=self._expected_rows)
            tmp_stream_store.create_group("/", 'meta', 'Sequence metadata')
            tmp_stream_store.create_vlarray('/meta', 'metas', atom=tables.ObjectAtom(), expectedrows=self._expected_rows)
            tmp_stream_store.create_group("/", 'info', 'Instance attributes')
            tmp_stream_store.create_vlarray('/info', 'dump', atom=tables.ObjectAtom(),expectedrows=1)
    ...
    def _restore_init(self):
        ...
        try:
            ...
            if all([group in group_list for group  in self._default_path_list]):
                tmp_instance_dict_bytes = tmp_stream_store_read.get_node(self._default_info_node_path).read()[0]
                tmp_instance_dict = pickle.loads(tmp_instance_dict_bytes)
                self.__dict__.update(tmp_instance_dict)
                tmp_stream_store_read.close()
                tmp_filters = tables.Filters(complib=self._compressor, complevel=self._default_complevel) if self._compressor else None
                tmp_stream_store = tables.open_file(self._stream_filepath, mode='a', filters=tmp_filters)
                self._stream_storer = tmp_stream_store
                ret = True
        except:
            pass
    ...
    def close(self,copy_filepath=None):
        tmp_instance_dict = {k:v for k,v in self.__dict__.items() if k not in ['_stream_filepath', '_stream_storer']}
        tmp_instance_dict_bytes = pickle.dumps(tmp_instance_dict)
        self._stream_storer.get_node(self._default_info_node_path).remove()
        self._stream_storer.create_vlarray('/info', 'dump', atom=tables.ObjectAtom(),expectedrows=1)
        self._stream_storer.get_node(self._default_info_node_path).append(tmp_instance_dict_bytes)
        ...
        if isinstance(acc_numbers,list):
            if len(acc_numbers)>0:
                if self._stream_map.index.isin(acc_numbers).sum() == len(acc_numbers):
                    ...
                    ret = MultiSequence(seq_list, name=self._name,aligned=self._aligned,metadata={'accession-numbers':'; '.join(acc_numbers)})
        return ret
        ...
        if not os.path.exists(fasta_fp):
            if isinstance(write_in_chunks,int):
                if write_in_chunks>=0:
                    chunks = len(self._stream_map) if write_in_chunks==0 else write_in_chunks
                    from Bio.Seq import Seq
                    with open(fasta_fp,'a') as fasta_handle:
                        chunk_counter = chunks
                        records_chunk = []
                        for sequence in self.iter_sequences():
                            tmp_record_metadata = sequence.metadata['description'] if 'description' in sequence.metadata.keys() else self._name
                            next_record = SeqIO.SeqRecord(Seq(sequence.text),sequence.name,description=tmp_record_metadata)
                            if chunk_counter > 1:
    ...
    def _retrieve_seq_by_seqid(self,seqid):
        tmp_seq_bytes = self._stream_storer.get_node(self._default_seqs_node_path)[seqid]
        return tmp_seq_bytes.decode(self._default_seq_encoding)

    def _retrieve_meta_by_seqid(self,seqid):
        tmp_meta_bytes = self._stream_storer.get_node(self._default_meta_node_path)[seqid]
        ...
        if isinstance(sequence, NucleotideMetabase):
            if isinstance(sequence,Nucleotide):
                if sequence.mode == self._mode:
                    if (sequence.name is not None) and (sequence.name not in self._stream_map.index):
                        ...
                        raise ValueError('Sequence name must be unique and have legnth > 0.')
                else:
    ...
    def _append_sequence(self,sequence_instance):
        ...
        if seqid == metaid:
            ...
            raise RuntimeError('Impossible condition. Stream file might have been externally modified!')
        return
            ...
            if multiseq.count>0:
                if multiseq.mode == self._mode:
                    for sequence in multiseq.sequences:
                        if (sequence.name is None) or (sequence.name in self._stream_map.index):
                            raise ValueError('Sequence name must be unique and have legnth > 0.')
    ...
    def append_string(self,name,mode,sequence_str,metadata_dict={}):
        if isinstance(name,str) and isinstance(sequence_str,str) and isinstance(metadata_dict,dict) and isinstance(mode,str):
            if mode == self._mode:
    ...
    def _insert_seq_vlarray(self,seq_data):
        ...
        self._stream_storer.get_node(self._default_seqs_node_path).append(seq_data_bytes)
        return self._stream_storer.get_node(self._default_seqs_node_path).nrows-1

    def _insert_meta_vlarray(self, metadata):
        metadata_bytes = pickle.dumps(metadata)
        self._stream_storer.get_node(self._default_meta_node_path).append(metadata_bytes)
