...
class DatabaseBase(DatabaseBackboneMetabase):
    ...
    def __repr__(self):
        ...
        if self.__storage_manager.state:
            ...
            tax_state = '+' if self.__storage_manager.element_state['taxonomy-sheet'] else '-'
            tree_state = '+' if self.__storage_manager.element_state['tree-object'] else '-'
            seq_state = ''.join(['+' if state_elem else '-' for state_elem in [self.__storage_manager.element_state['sequence-representative'], self.__storage_manager.element_state['sequence-aligned'],self.__storage_manager.element_state['sequence-accession']]])
            repr_str = "<{}:[{}], Taxa: [{}], Ranks: [{}], Taxonomy/Sequence/Tree: [{}/{}/{}]>".format(class_name, state, total_taxa, ranks, tax_state, seq_state, tree_state)
        else:
    ...
    def take_tids_by_rank(self, levels=None, iterator=False, flatten=False, mode='dict'):
        ...
        if self.storage_manager.state == 1:
            if levels is None:
                ...
                target_rank_levels = np.asarray(levels)
            if all([rank in self.avail_ranks for rank in target_rank_levels]):
    ...
    def find_sub_tids_by_tid(self, ids=None, ter_rank=None, flatten=False, mode='frame'):
        ...
        if self.storage_manager.state == 1:
            ...
            if self.xtid.isin(target_unique_ids).sum() == len(target_unique_ids):
                ...
                target_ranks = focus_map2tid.columns[focus_map2tid.isin(target_unique_ids).any(axis=0)].values.tolist()
                            ...
                            if tid != 0 and tid in target_unique_ids:
                                ...
                                if tmp_subranks:
                                    if ter_rank is None:
                                        ...
                                        tmp_tids_map[tid].extend(np.unique(tmp_subtids.values[tmp_subtids.values > 0].flatten()))
                                    else:
                                        tmp_subtids_adj = get_rank_upto(tmp_subranks[::-1], ter_rank, True)
                                        if tmp_subtids_adj:
                                            tmp_subtids = target_map2tid.loc[subtids, tmp_subtids_adj]
                                            tmp_tids_map[tid].extend(np.unique(tmp_subtids.values[tmp_subtids.values > 0].flatten()))
                    return to_mode({tid: np.asarray(rids, dtype=map2tid.dtypes[0]) for tid, rids in tmp_tids_map.items()}, mode, target_ids)
                else:
                    tmp_tids_flat = set()
                    for rank in target_ranks:
                        ...
                        for tid, subtids in groupby.groups.items():
                            if tid != 0 and tid in target_unique_ids:
                                ...
                                if tmp_subranks:
                                    if ter_rank is None:
                                        tmp_subtids = target_map2tid.loc[subtids, tmp_subranks]
                                        tmp_tids_flat.update(np.unique(tmp_subtids.values[tmp_subtids.values > 0].flatten()).tolist())
                                    else:
                                        tmp_subtids_adj = get_rank_upto(tmp_subranks[::-1], ter_rank, True)
                                        if tmp_subtids_adj:
                                            tmp_subtids = target_map2tid.loc[subtids, tmp_subtids_adj]
            ...
            if self.xtid.isin(target_unique_ids).sum() == len(target_unique_ids):
                ...
                def map_generator():
                    ...
                    if not subs:
                        ...
                        partial_map = map2tid[map2tid[map2tid.columns[map2tid.columns != 'tid']].isin(target_ids).any(axis=1)]
                        tid_rid_gps_col = {rank: partial_map.groupby(rank, sort=False).groups for rank in self.avail_ranks}
                        for tid in target_ids:
                            point_rank = None
                            for rank in self.avail_ranks:
                                if tid in tid_rid_gps_col[rank].keys():
                                    point_rank = rank
                                    break
                            yield tid, np.asarray([]) if point_rank is None else tid_rid_gps_col[point_rank].get(tid, pd.Index([])).values
    ...
    def find_tid_by_rid(self, ids=None, levels=None, flatten=False, method='legal', mode='frame'):
        ...
        if self.storage_manager.state == 1:
            ...
            if self.xrid.isin(target_unique_ids).sum() == len(target_unique_ids):
                map2tid = self.storage_manager.retrieve_data_by_element('map-rep2tid')
                if map2tid.columns.isin(target_unique_ranks).sum() == len(target_unique_ranks):
                    if not flatten:
                        if len(target_ranks) > 1:
                            if method == 'legal':
                                retriever = lambda rid_row: rid_row[rid_row > 0].values
                                tid_rid_product = map2tid.loc[target_unique_ids, target_unique_ranks].apply(retriever, axis=1)
                                return to_mode(tid_rid_product.loc[target_ids], mode, None)
                            elif method == 'asis':
                                tid_rid_product = map2tid.loc[target_unique_ids, target_unique_ranks]
                                return to_mode(tid_rid_product.loc[target_ids, target_ranks], mode, None)
                            else:
                                raise ValueError('Invalid `method` value is provided.')
                        else:
                            if method == 'legal':
                                tid_array = map2tid.loc[target_unique_ids, target_unique_ranks[0]]
                                tid_rid_product = tid_array[tid_array > 0]
                                return to_mode(tid_rid_product.loc[target_ids], mode, None)
                            elif method == 'asis':
                                tid_rid_product = map2tid.loc[target_unique_ids, target_unique_ranks[0]]
                                return to_mode(tid_rid_product.loc[target_ids], mode, None)
                            else:
                                raise ValueError('Invalid `method` value is provided.')
                    else:
                        flat_tid_rid_product = np.unique(map2tid.loc[target_unique_ids, target_unique_ranks].values.ravel())
                        if method == 'legal':
                            tid_rid_product = flat_tid_rid_product[flat_tid_rid_product > 0]
                        elif method == 'asis':
                ...
                if 'stat-reps' in self.storage_manager.active_elements:
                    ...
                    for col in rid_stat_cols:
                        if len(target_inc) > 0 and len(target_exc) > 0:
                            ...
                            target_cols.append(col)
                    if ids is None:
                        product = self.storage_manager.retrieve_data_by_element('stat-reps', target_cols)
                    else:
                        tmp_product = self.storage_manager.get_element_data_by_ids('stat-reps', target_unique_ids)
                ...
                if 'stat-taxs' in self.storage_manager.active_elements:
                    ...
                    for col in rid_stat_cols:
                        if len(target_inc) > 0 and len(target_exc) > 0:
                            ...
                            target_cols.append(col)
                    if ids is None:
                        product = self.storage_manager.retrieve_data_by_element('stat-taxs', target_cols)
                    else:
                        tmp_product = self.storage_manager.get_element_data_by_ids('stat-taxs', target_unique_ids)
                        product = tmp_product.loc[target_ids, target_cols]
                    if not isinstance(include, str):
                        return product
                    else:
