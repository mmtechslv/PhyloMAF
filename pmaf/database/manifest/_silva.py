...
class DatabaseSILVA(DatabaseTaxonomyMixin,DatabaseSequenceMixin,DatabasePhylogenyMixin,DatabaseAccessionMixin,DatabaseBase):
    ...
    def __process_tax_acs_map(cls, storage_manager, taxonomy_map_csv_fp):
        ...
        def parse_silva_taxonomy_map(taxonomy_prior):
            ...
            tax_sheet_kingdom_fix.loc[tmp_taxonomy_sheet.iloc[:, 0].isin(['Archaea']), 'K'] = 'Archaea'
            tax_sheet_kingdom_fix.loc[tmp_taxonomy_sheet.iloc[:, 0].isin(['Bacteria']), 'K'] = 'Bacteria'
            tax_sheet_kingdom_fix.columns = MAIN_RANKS
            tmp_taxonomy_sheet = tmp_taxonomy_sheet[~tmp_taxonomy_sheet.iloc[:, 0].isin(['Archaea', 'Bacteria'])]
        ...
        def produce_sequence_accession(index_mapper, dropped_taxa):
            ...
            tmp_accessions.insert(loc=1, column='ncbi', value=tmp_accessions.loc[:, 'silva'].map(lambda x: x[:x.find('.')]))
        ...
        storage_manager.commit_to_storage('taxonomy-prior', produce_taxonomy_prior(tmp_taxonomy_prior, index_mapper, removed_rids))
        ...
        tmp_recap = summarizer.append_recaps(tmp_recap,{'molecule-type':'DNA','strand-orientation':'forward','gene':'rRNA','gene-target-region':'16S|18S'})
