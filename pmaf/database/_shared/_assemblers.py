...
def finalize_storage_construction(storage_manager, stamp_data, prior_recap, **kwargs):
    ...
    **kwargs :
    ...
    storage_manager.commit_to_storage(
        "metadata-db-summary", produce_metadata_db_summary(final_recap)
    )
    ...
    if compress:
        ...
        if not storage_manager.compress_storage(
            complevel=complevel, complib=complib, overwrite=overwrite
        ):
    ...
    if storage_manager.state == -1:
        ...
        if len(active_elements) > 0:
            repseq_elements = [
                ...
                if get_element_index_type(element) == "map-interx-repseq"
            ]
            taxon_elements = [
                element
                for element in active_elements
                if get_element_index_type(element) == "map-interx-taxon"
            ]
            ...
            def make_interxmap(target_elements, storage_manager):
                ...
                internal_index_map = (
                    internal_index_map.reset_index(name="target")
                    .set_index("target")
                    .rename({"index": first_element}, axis=1)
                )
                if len(target_elements) > 1:
                    for element in target_elements[1:]:
                        next_elemenet_index_map = storage_manager.get_index_by_element(
                            element
                        )
                        next_elemenet_index_map = (
                            next_elemenet_index_map.reset_index(name="target")
                            .set_index("target")
                            .rename({"index": element}, axis=1)
                        )
                        internal_index_map = internal_index_map.join(
                            next_elemenet_index_map, how="left"
                        )
            ...
            return {
                "map-interx-repseq": make_interxmap(repseq_elements, storage_manager),
                "map-interx-taxon": make_interxmap(taxon_elements, storage_manager),
            }
        else:
...
def rebuild_phylo(tree_object):
    ...
    duplicated_nodes = [
        tree_object.iter_search_nodes(name=node_name) for node_name in duplicated_names
    ]
...
def make_tree_map(tree_object):
    ...
    tree_map = pd.DataFrame.from_records(
        uid_map_list, columns=["uid", "pid"], index=["uid"]
    )
...
def reconstruct_taxonomy(master_taxonomy_sheet_df, index_mapper, reject_taxa=None):
    ...
    tmp_dropped_taxa = master_taxonomy_sheet_df.index[
        ~master_taxonomy_sheet_df.index.isin(tmp_master_tax_sheet_loose.index)
    ]
    ...
    taxdf_dd = pd.DataFrame(
        data=tmp_taxa_series_list,
        index=range(1, group_i),
        columns=tmp_master_tax_sheet.columns,
    )

    tmp_rep2gid = pd.DataFrame.from_dict(
        prior2group, orient="index", columns=["deduplication"]
    )
    ...
    if avail_ranks[-1] == "s":
        ...
        for taxa_series, repseq_ids in groupby.groups.items():
            ...
            group_i = group_i + 1
        taxdf_dd_ds = pd.DataFrame(
            data=tmp_taxa_series_list, index=range(1, group_i), columns=avail_ranks
        )
    ...
    for rank in avail_ranks:
        ...
        rank_tids = taxdf_dd_ds[taxdf_dd_ds[none_ranks].isna().all(axis=1)].dropna(
            subset=[rank]
        )[group_ranks]
        ...
        for taxa, tids in groupby.groups.items():
            ...
            if pd.notna(taxa_tuple[-1]):
                taxa_dict = {
                    avail_ranks[ki]: [taxa_tuple[ki]] for ki in range(target_rank_i + 1)
                }
                target_rank_id = rank_tids[
                    rank_tids.isin(taxa_dict).all(axis=1)
                ].index.values.tolist()
                if len(target_rank_id) == 0:
                    target_rank_id = [
                        taxdf_dd_ds.index.max() + len(novel_tids_dict) + 1
                    ]
                    ...
                    raise RuntimeError(
                        "Impossible scenario. Taxa at given rank cannot have more than one repseq. Invalid unduplication step."
                    )
    ...
    transformation_details = {
        "map-rep2tid": rep2tid_map,
        "removed-rids": removed_rids,
        "changes": tmp_rep2gid,
        "novel-tids": list(novel_tids_dict.keys()),
        "avail-ranks": avail_ranks,
    }
    return (
        final_taxonomy_sheet.applymap(lambda x: "" if pd.isna(x) else x),
        transformation_details,
    )
...
def make_rid_index_mapper(rids_index):
    ...
    """
    return (
        rids_index.to_series(index=range(1, rids_index.shape[0] + 1), name="rids")
        .reset_index()
        .set_index("rids")
        .iloc[:, 0]
    )
...
def make_column_details(storage_manager):
    ...
    for element_key in storage_manager.active_elements:
        ...
        if get_element_type(element_key) == "fixed":
            ...
            tmp_df = next(
                storage_manager.retrieve_data_by_element(element_key, chunksize=2)
            )
        if isinstance(tmp_df, pd.DataFrame):
            tmp_column_summary["columns-{}".format(element_key)] = "|".join(
                list(tmp_df.columns)
            )
...
def produce_rep_stats(storage_manager, chunksize):
    ...
    repseq_generator = storage_manager.retrieve_data_by_element(
        "sequence-representative", chunksize=chunksize
    )

    yield {
        "index_columns": ["index"],
        "max_rows": total_repseqs,
    }, get_stats_for_sequence_record_df(next(repseq_generator))
...
def produce_tax_stats(storage_manager, novel_tids):
    ...
    tax_stats = pd.DataFrame.from_dict(
        tid_subseqs_stat,
        orient="index",
        columns=["novel", "subseqs", "singleton", "rank"],
    )
    yield None, None
    yield tax_stats
...
def make_repseq_map_generator(transformation_details, chunksize=500):
    ...
    """
    tid_index = (
        transformation_details["map-rep2tid"]["tid"].unique().tolist()
        + transformation_details["novel-tids"]
    )
    rid_index = transformation_details["map-rep2tid"].index.values.tolist()

    repseq_map_dict = {
        tid: np.asarray([0] * len(rid_index), dtype="int8") for tid in tid_index
    }
        ...
        for tid, rids in groupby.groups.items():
            ...
            if tid != "":
                for rid in rids:
                    tmp_rid_dict[
                        rid
                    ] = 0b100  # 0b100 corresponds to any related sequence
                repseq_map_dict[tid] = np.bitwise_or(
                    repseq_map_dict[tid],
                    np.asarray([tmp_rid_dict[rid] for rid in rid_index], dtype="int8"),
                )
    ...
    pre_state_dict = {
        "index": max(list(map(len, tid_index))),
        "max_rows": len(tid_index),
    }

    yield pre_state_dict, pd.DataFrame.from_records(
        repseq_map_array[:chunksize], index=tid_index[:chunksize], columns=rid_index
    ).astype(np.dtype("int8"))

    for chunk_i in range(chunksize, len(tid_index)):
        yield pd.DataFrame.from_records(
            repseq_map_array[chunk_i : chunk_i + chunksize],
            index=tid_index[chunk_i : chunk_i + chunksize],
            columns=rid_index,
        ).astype(np.dtype("int8"))
