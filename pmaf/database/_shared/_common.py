...
def to_mode(result_obj, mode="array", order=None):
    ...
    if mode == "array":
                    ...
                    if isinstance(result_obj, pd.DataFrame):
                        if result_obj.shape[1] > 1:
                            if result_obj.index.has_duplicates:
                                result_obj_unq = result_obj[
                                    result_obj.index.duplicated()
                                ]
                                return (
                                    result_obj_unq.loc[order]
                                    .apply(tuple, axis=1)
                                    .values
                                )
                            else:
                            ...
                            if result_obj.index.has_duplicates:
                                ...
                                return result_obj.reindex(
                                    index=order, fill_value=pd.NA
                                ).values
            ...
            if order is None:
                ...
                if result_obj.index.isin(order).all():
                    if result_obj.index.has_duplicates:
                        ...
                        return result_obj.reindex(
                            index=order, fill_value=np.asarray([])
                        )
                else:
                    raise ValueError("`order` index does not match `result_obj`.")
        elif isinstance(result_obj, (list, tuple)):
        ...
        if isinstance(result_obj, pd.DataFrame):
            ...
            if isinstance(result_obj, np.ndarray) or np.isscalar(
                result_obj
            ):  # Fallback
                return result_obj
            else:
...
def verify_tax_format(tax_format):
    ...
    target_placeholders = [
        fmt[1] for fmt in Formatter().parse(tax_format) if fmt[1] is not None
    ]
