...
class DatabaseStorageManager:
    def __init__(self, hdf5_filepath, storage_name, force_new=False):
        ...
        self._interxmap_cache = {'map-interx-taxon':None,'map-interx-repseq':None}
    ...
    def __init_manager(self):
        """
        This function initiates storage manager by making it active for working.
        It also loads metadata-db-info into memory which is required to let know other package _components about what kind of data is available in the storage file.

        Returns:
            Nothing but raises error if initiation was not success.
        """
        try:
            self._db_info_cache = pd.read_hdf(self._hdf5_filepath, DATABASE_HDF5_STRUCT["metadata-db-info"])
            self._init_state = 1
            ...
            raise RuntimeError('Failed to initiate manager. Reading HDF5 was not successful.')
    ...
    def initiate_memory_cache(self,level=1):
        ...
        if self._init_state==1:
            ...
            if level>=1:
                if self._db_info_cache['map-interx-taxon'] and self._db_info_cache['map-interx-repseq']:
                    self._interxmap_cache['map-interx-repseq'] = tmp_storer.select(DATABASE_HDF5_STRUCT['map-interx-repseq'])
                    self._interxmap_cache['map-interx-taxon'] = tmp_storer.select(DATABASE_HDF5_STRUCT['map-interx-taxon'])
                    self._db_summary = tmp_storer.select(DATABASE_HDF5_STRUCT['metadata-db-summary'])
                    ret = 1
            if level>=2:
                if self._db_info_cache['taxonomy-sheet']:
                    self._supplement_cache['taxonomy-sheet'] = tmp_storer.select(DATABASE_HDF5_STRUCT['taxonomy-sheet'])
                    ret = 2
            if level>=3:
                for element_key in filter_elements_by('map-',['map-interx-repseq','map-interx-taxon']):
                    if self._db_info_cache[element_key]:
                        self._supplement_cache[element_key] = tmp_storer.select(DATABASE_HDF5_STRUCT[element_key])
                        ret = 3
            tmp_storer.close()
            if level>=4:
                if self._db_info_cache['tree-object']:
                    tmp_storer = tmp_storer.get_handle_by_element('tree-object')
                    tree_object_bytes = tmp_storer.get_node(DATABASE_HDF5_STRUCT['tree-object']).read()[0]
    ...
    def validate_storage(hdf5_filepath,storage_name):
        ...
        try:
            ...
            if tmp_storer.title == storage_name:
                ...
                tmp_storer.close()
                title_verify = all([title in title_list[1:] for title in DATABASE_HDF5_STRUCT.keys()])
                path_verify = all([path in path_list[1:] for path in DATABASE_HDF5_STRUCT.values()])
    ...
    def __close_storer(self):
        """
        This method closes storer and releases storage file for reopening.
        Returns:
    ...
    def __open_as_tables(self,mode='r'):
        if self._init_state:
            self.__close_storer()
            try:
    ...
    def __build_storage(self):
        """
        This method builds new storage element and generates its internal structure. It also fill necessary defaults to make working storage file.
        Returns:
            Nothing but will raise error if something goes wrong.
        """
        try:
            tmp_storer = tables.open_file(self._hdf5_filepath,mode='w',title=self._storage_name)
            ...
            tmp_storer.create_vlarray('/tre/master', 'value', title='bytes', atom=tables.VLUnicodeAtom())
            tmp_storer.create_vlarray('/tre/parsed', 'value', title='bytes', atom=tables.VLUnicodeAtom())
            tmp_storer.create_vlarray('/tre/pickled', 'value', title='bytes', atom=tables.ObjectAtom())
            ...
            tmp_element_key_defaults = ['tree-parsed', 'tree-prior', 'tree-object','taxonomy-prior','taxonomy-sheet','sequence-representative',
                                        'sequence-aligned','sequence-accession','metadata-db-summary','metadata-db-stamp',
                                        'map-interx-taxon', 'map-interx-repseq', 'map-rep2tid','map-repseq','map-tree','stat-reps','stat-taxs']
            ...
            tmp_storer.put(DATABASE_HDF5_STRUCT['metadata-db-info'], tmp_element_defaults_series, format='fixed')
            tmp_storer.get_node(DATABASE_HDF5_STRUCT['metadata-db-info'])._v_title = 'metadata-db-info'
    ...
    def commit_to_storage(self,element_key,product_generator):
        ...
        if self._init_state == -1 :
            ...
            raise RuntimeError('Data base was already stamped so it is not possible to commit again.')


    def __put_to_storage(self,element_key,product_generator):
        """
        ...
        Returns:
            Last chunk or last product
        """
        ret = None
        try:
            if element_key in ['sequence-representative','sequence-aligned']:
                if self.__open_as_pandas('a'):
                    product_inits, product_generator_first_chunk = next(product_generator)
                    max_rows = product_inits.pop('max_rows')
                    self._storer.put(DATABASE_HDF5_STRUCT[element_key], product_generator_first_chunk, format='table', data_columns=True,min_itemsize=product_inits,index=False)
                    last_chunk = product_generator_first_chunk
                    for next_chunk in product_generator:
                        self._storer.append(DATABASE_HDF5_STRUCT[element_key], next_chunk, format='table', data_columns=True,min_itemsize=product_inits,expectedrows=max_rows,index=False)
                        last_chunk = next_chunk
                    self._storer.get_node(DATABASE_HDF5_STRUCT[element_key])._v_title = element_key
                    self._storer.create_table_index(DATABASE_HDF5_STRUCT[element_key], columns=['index'], optlevel=9, kind='full')
                    ret = last_chunk
            elif element_key in ['sequence-accession', 'taxonomy-prior','taxonomy-sheet','metadata-db-summary', 'map-rep2tid','map-repseq','metadata-db-history','stat-reps','stat-taxs']:
                if self.__open_as_pandas('a'):
                    product_inits, product_generator_first_chunk = next(product_generator)
                    if (product_inits is not None) and (product_generator_first_chunk is not None):
                        max_rows = product_inits.pop('max_rows')
                        x_columns = product_inits.pop('index_columns')
                        min_itemsize = product_inits if len(product_inits)>0 else None
                        self._storer.put(DATABASE_HDF5_STRUCT[element_key], product_generator_first_chunk, format='table', data_columns=True, min_itemsize=min_itemsize,index=False)
                        last_chunk = product_generator_first_chunk
                        for next_chunk in product_generator:
                            self._storer.append(DATABASE_HDF5_STRUCT[element_key], next_chunk, format='table', data_columns=True, min_itemsize=min_itemsize, expectedrows=max_rows, index=False)
                            last_chunk = next_chunk
                        self._storer.get_node(DATABASE_HDF5_STRUCT[element_key])._v_title = element_key
                        self._storer.create_table_index(DATABASE_HDF5_STRUCT[element_key], columns=x_columns, optlevel=9, kind='full')
                        ret = last_chunk
                    else:
                        product_product_whole = next(product_generator)
                        self._storer.put(DATABASE_HDF5_STRUCT[element_key], product_product_whole, format='table',data_columns=True)
                        self._storer.get_node(DATABASE_HDF5_STRUCT[element_key])._v_title = element_key
                ...
                if self.__open_as_tables('a'):
                    ...
                    self._storer.get_node(DATABASE_HDF5_STRUCT[element_key]).append(product_product_whole_utf8)
                    ret = product_product_whole
                ...
                if self.__open_as_tables('a'):
                    ...
                    self._storer.get_node(DATABASE_HDF5_STRUCT[element_key]).append(product_product_whole_bytes)
                    ret = product_product_whole
            elif element_key in ['map-interx-taxon','map-interx-repseq','map-tree']:
                if self.__open_as_pandas('a'):
                    _, _ = next(product_generator)
                    product_product_whole = next(product_generator)
                    self._storer.put(DATABASE_HDF5_STRUCT[element_key], product_product_whole, format='fixed',data_columns=True)
                    self._storer.get_node(DATABASE_HDF5_STRUCT[element_key])._v_title = element_key
                    ret = product_product_whole
        except:
        ...
        if self._init_state == -1:
            ...
            if self.__open_as_tables('r'):
                ...
                for node in self._storer.walk_nodes():
                    if node._v_title in DATABASE_HDF5_STRUCT.keys() and not node._v_title.startswith('root-'):
                        if node._v_nchildren > 0:
                            if node._v_children.values()[0]._v_title == 'bytes':
                                ...
                                title_list.append(node._v_title)
                if self._db_info_cache.index.isin(title_list).sum() != self._db_info_cache.sum():
                    raise RuntimeError('Cannot imprint. Possible storage file corruption.')
                self.__open_as_pandas('a')
                self._db_info_cache['metadata-db-stamp'] = True
                self._storer.put(DATABASE_HDF5_STRUCT['metadata-db-info'], self._db_info_cache, format='fixed')
                self._storer.get_node(DATABASE_HDF5_STRUCT['metadata-db-info'])._v_title = 'metadata-db-info'

                self._storer.put(DATABASE_HDF5_STRUCT['metadata-db-stamp'], pd.Series(stamp_dict), format='fixed')
                self._storer.get_node(DATABASE_HDF5_STRUCT['metadata-db-stamp'])._v_title = 'metadata-db-stamp'
        return

    def __set_handle_by_element(self, element_key):
    ...
    def __get_coordinates_for_element_by_ids(self, element_key, ids):
        ...
        if self._init_state == 1:
            ...
            if valid_ids.sum() == len(ids):
                tmp_target_index_map = self._interxmap_cache[tmp_interxmap_type][element_key][valid_ids]
                ret = pd.Index(tmp_target_index_map.values)
            else:
                raise ValueError('Invalid identifiers are provided.')
        else:
            index_coord = self._storer.select_column(DATABASE_HDF5_STRUCT[element_key], 'index')
            ...
            if self._db_info_cache[element_key] or self._init_state == -1:
                if get_element_mode(element_key) == 2:
                    ...
                    if condition is None:
                        return self._storer.select_column(DATABASE_HDF5_STRUCT[element_key], 'index')
                    else:
                        if isinstance(condition,str):
                            return self._storer.select(DATABASE_HDF5_STRUCT[element_key], "columns == index & {}".format(condition))
                        else:
                            raise ValueError("Invalid condition parameter is provided. `condition` must have string type.")
                else:
    ...
    def retrieve_data_by_element(self,element_key,columns=None,chunksize=None):
        ...
        if self._init_state:
            if self._db_info_cache[element_key]:
                if get_element_mode(element_key) == 2:
                    if (chunksize is not None) and get_element_type(element_key) != 'table':
                        raise ValueError('Error! Attempt to use `chunksize` on fixed tables.')
                    if self._supplement_cache.get(element_key,None) is None:
                        self.__set_handle_by_element(element_key)
                        if columns is None:
                            product = self._storer.select(DATABASE_HDF5_STRUCT[element_key],chunksize=chunksize)
                        else:
                            product = self._storer.select(DATABASE_HDF5_STRUCT[element_key], "columns in ['index', {}]".format(', '.join(["'{}'".format(column) for column in columns])), chunksize=chunksize)
                        return self.__fix_table(element_key, product) if chunksize is None else fixed_product_generator(product)
                    else:
                        if columns is None:
                            return self.__fix_table(element_key,self._supplement_cache[element_key])
                        else:
                            return self.__fix_table(element_key,self._supplement_cache[element_key][columns])

                else:
                    if self._supplement_cache.get(element_key,None) is None:
                        self.__set_handle_by_element(element_key)
                        return self._storer.get_node(DATABASE_HDF5_STRUCT[element_key]).read()[0]
                    else:
    ...
    def get_element_data_by_ids(self, element_key, ids):
        if self._init_state:
            if self._db_info_cache[element_key]:
                ...
                if (get_element_mode(element_key) == 2) and (get_element_index_type(element_key) != None):
                    if self._supplement_cache.get(element_key,None) is None:
                        self.__set_handle_by_element(element_key)
                        coord = self.__get_coordinates_for_element_by_ids(element_key, target_ids)
                        product = self._storer.select(DATABASE_HDF5_STRUCT[element_key], coord)
                        return self.__fix_table(element_key,product)
                    else:
                        valid_ids = self._supplement_cache[element_key].index.isin(target_ids)
                        if valid_ids.sum() == len(target_ids):
                            return self.__fix_table(element_key, self._supplement_cache[element_key].loc[target_ids])
                        else:
                            raise ValueError('Invalid identifiers provided.')
                else:
                    raise ValueError("Required element was not intended to be used via indexing. Use `retrieve_data_by_element` instead.")
            else:
                raise ValueError("Invalid element requested.")
        else:
            raise RuntimeError("Storage manager must be initiated.")
