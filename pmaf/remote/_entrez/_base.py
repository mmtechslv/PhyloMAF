    ...
    def wrapper(instance_self, *args, **kwargs):
        ...
        **kwargs :
        ...
        while retry:
            ...
            if (last_runtime < 1) and (
                instance_self._max_requests == instance_self._get_request_count()
            ):
                time.sleep(1 - last_runtime)
                last_runtime = 0
                instance_self._reset_request_counter()
            if (last_runtime >= 1) or (
                instance_self._max_requests == instance_self._get_request_count()
            ):
                time.sleep(1)
                last_runtime = 0
            ...
            request_log = {
                "request": method_pass.__name__,
                "attempts": instance_self._get_attempts() - attempt_counter,
                "runtime": instance_self._get_last_runtime(),
            }
...
class EntrezBase(EntrezBackboneMetabase, IdiomaticBase):
    ...
    def __repr__(self):
        ...
        repr_str = "<{}:[{}], API:[{}], Email: [{}]>".format(
            class_name, state, api_state, email
        )
        return repr_str
    ...
    def _add_request_log(self, method_name):
        ...
        """
        self.__request_logs.append(
            {
                "Request": method_name,
                "Cumulative Runtime": self.__last_runtime,
                "Request Count": self.__request_counter,
            }
        )
        return
    ...
    def _request_sequence_feature_table_by_acc_id(self, accession_id, handle=False):
        ...
        """
        efetch_handle = self._BioEntrez.efetch(
            db="nuccore", id=accession_id, rettype="ft", retmode="text"
        )
        if handle:
    ...
    def _request_chromosomes_by_genome_id(self, genome_id):
        ...
        """
        with self._BioEntrez.elink(
            dbfrom="genome",
            db="nuccore",
            id=genome_id,
            idtype="acc",
            term="gene+in+chromosome[prop]",
        ) as elink_handle:
    ...
    def _request_genomes_by_taxid(self, taxid):
        ...
        """
        with self._BioEntrez.elink(
            dbfrom="taxonomy", db="genome", id=taxid
        ) as elink_handle:
            elink_record = self._BioEntrez.read(elink_handle)
        return elink_record
    ...
    def _request_taxid_by_query(self, query):
        ...
        """
        with self._BioEntrez.esearch(
            db="taxonomy", retmax=self._retmax, term=query
        ) as esearch_handle:
