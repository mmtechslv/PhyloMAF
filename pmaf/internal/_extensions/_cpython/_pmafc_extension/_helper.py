...
def make_sequence_record_tuple(sequence_id, sequence_str):
    ...
    '''
    ambiguous_count = [(sequence_str.count(b), len(bb)) for b, bb in IUPAC_AMBIGUOUS.items() if b in sequence_str]
    total_ambiguous = reduce(lambda x, y: x + y, [ae[0] for ae in ambiguous_count]) if len(ambiguous_count) > 0 else 0
