...
import numpy as np
import biom
from typing import Union, Sequence, TypeVar, Tuple, Any
from pmaf.internal._typing import GenericIdentifier, Mapper, Optional

T = TypeVar('T', bound='RepTaxonomy')
...
class RepTaxonomy(EssentialBackboneBase, EssentialFeatureMetabase):
			...
			if path.isfile(taxonomy):
				...
				if file_extension in ['.csv', '.tsv']:
					if taxonomy_columns is None:
						tmp_taxonomy = pd.read_csv(taxonomy, sep=kwargs.pop('sep', ','),
						...
						if isinstance(taxonomy_columns, int):
							tmp_taxonomy = pd.read_csv(taxonomy, sep=kwargs.pop('sep', ','),
													   header=kwargs.pop('header', 'infer'),
													   index_col=kwargs.pop('index_col', None)).iloc[:,
										   taxonomy_columns]
						else:
							tmp_taxonomy = pd.read_csv(taxonomy, sep=kwargs.pop('sep', ','),
													   header=kwargs.pop('header', 'infer'),
													   index_col=kwargs.pop('index_col', None)).loc[:, taxonomy_columns]
	...
	def __load_biom(cls,
		...
		if biom_file.metadata(axis='observation') is not None:
			...
			avail_col_names = [colname for tax_name in BIOM_TAXONOMY_NAMES for colname in col_names_low if
							   colname[::-1].find(tax_name[::-1]) < 3 and colname[::-1].find(tax_name[::-1]) > -1]
	...
	def _merge_features_by_map(self,
		...
		Args:
			...
			done: Whether merging was completed or not. Compatibility.
			**kwargs: Compatibility

		"""
		if done:
			if len(map_dict) > 0:
				return self._ratify_action('_merge_features_by_map', map_dict,
										   _annotations=self.__internal_taxonomy.loc[:, 'lineage'].to_dict(), **kwargs)
		else:
			raise NotImplementedError

	def drop_feature_by_id(self,
						   ids: GenericIdentifier,
						   **kwargs: Any) -> Optional[GenericIdentifier]:
	...
	def get_taxonomy_by_id(self,
						   ids: Optional[GenericIdentifier] = None) -> pd.DataFrame:
		"""Get taxonomy :class:`~pandas.DataFrame` by feature `ids`.

		Args:
			ids: Either feature indices or None for all.
	...
	def get_lineage_by_id(self,
		...
		Args:
			ids: Either feature indices or None for all.
		  	missing_rank: If True will generate prefix like `s__` or `d__`
		  	desired_ranks: List of desired ranks to generate. If False then will generate all main ranks
		  	drop_ranks: List of ranks to drop from desired ranks. This parameter only useful if `missing_rank` is True
		  	**kwargs: Compatibility.
		...
		if total_valid_rids == len(target_ids):
			return generate_lineages_from_taxa(self.__internal_taxonomy.loc[target_ids], missing_rank,
											   tmp_desired_ranks, drop_ranks)
		elif total_valid_rids < len(target_ids):
			return generate_lineages_from_taxa(self.__internal_taxonomy.loc[np.unique(target_ids)], missing_rank,
											   tmp_desired_ranks, drop_ranks)
		else:
			raise ValueError('Invalid feature ids are provided.')

	def find_features_by_pattern(self,
								 pattern_str: str,
		...
		Args:
			pattern_str: Pattern to search for
		  	case_sensitive: Case sensitive mode
		...
		return self.__internal_taxonomy[
			self.__internal_taxonomy.loc[:, 'lineage'].str.contains(pattern_str, case=case_sensitive,
																	regex=regex)].index.values

	def drop_features_without_taxa(self,
								   **kwargs: Any) -> Optional[GenericIdentifier]:
	...
	def drop_features_without_ranks(self,
									ranks: Sequence[str],
									any: bool = False,
									**kwargs: Any) -> Optional[GenericIdentifier]:  # Done
		"""Remove features that do not contain `ranks`

		Args:
			ranks: Ranks to look for
		  	any: If True removes feature with single occurrence of missing rank. If False all `ranks` must be missing.
		  	**kwargs: Compatibility
	...
	def merge_duplicated_features(self,
								  **kwargs: Any) -> Optional[Mapper]:
	...
	def merge_features_by_rank(self,
							   level: str,
							   **kwargs: Any) -> Optional[Mapper]:
		"""Merge features by taxonomic rank/level

		Args:
			level: Taxonomic rank/level to use for merging.
			**kwargs: Compatibility
		"""
		ret = {}
	...
	def find_features_without_taxa(self) -> np.ndarray:
		...
		return self.__internal_taxonomy.loc[self.__internal_taxonomy.loc[:, VALID_RANKS].
												agg(lambda rank: len(''.join(map(lambda x: (str(x or '')), rank))),
													axis=1) < 1].index.values
	...
	def get_subset(self,
		...
		if not self.xrid.isin(target_rids).sum() == len(target_rids):
			raise ValueError('Invalid feature ids are provided.')
		return type(self)(taxonomy=self.__internal_taxonomy.loc[target_rids, 'lineage'], metadata=self.metadata,
						  name=self.name)

	def _export(self,
				taxlike: str = 'lineage',
				ascending: bool = True,
				**kwargs: Any) -> Tuple[pd.Series, dict]:
		"""Creates taxonomy for export

		Args:
			taxlike: Generate taxonomy in format(currently only `lineage` is supported.)
			ascending: Sorting
			**kwargs: Compatibility
		"""
		if taxlike == 'lineage':
	...
	def __fix_taxon_names(self) -> None:
		...
		self.__internal_taxonomy.loc[:, VALID_RANKS] = self.__internal_taxonomy.loc[:, VALID_RANKS].applymap(
			taxon_fixer)

	def __reconstruct_internal_lineages(self) -> None:
		"""Reconstruct the internal lineages."""
		self.__internal_taxonomy.loc[:, 'lineage'] = generate_lineages_from_taxa(self.__internal_taxonomy, True,
																				 self.__avail_ranks, False)
	...
	def __init_internal_taxonomy(self,
		...
		Args:
			taxonomy_data: Incoming parsed taxonomy data
			taxonomy_notation: Taxonomy lineage notation style. Can be one of :const:`pmaf.internals._constants.AVAIL_TAXONOMY_NOTATIONS`
			order_ranks: List with the target rank order. Default is set to None. The 'silva' notation require `order_ranks`.
			**kwargs: Compatibility
		"""
	...
	def __init_taxonomy_from_lineages(self,
									  taxonomy_series: pd.Series,
									  taxonomy_notation: Optional[str],
									  order_ranks: Optional[Sequence[str]]) -> pd.DataFrame:  # Done
		"""Main method that produces taxonomy dataframe from lineages

		Args:
			taxonomy_series: :class:`pandas.Series` with taxonomy lineages
			taxonomy_notation: Taxonomy lineage notation style. Can be one of :const:`pmaf.internals._constants.AVAIL_TAXONOMY_NOTATIONS`
			order_ranks:  List with the target rank order. Default is set to None. The 'silva' notation require `order_ranks`.

		Returns:
			:class:`~pandas.DataFrame` with taxonomy split into ranks.
		...
		if taxonomy_notation in AVAIL_TAXONOMY_NOTATIONS:  # Check if taxonomy is known and is available for parsing. Otherwise indentify_taxon_notation() will try to identify notation
			...
			sample_taxon = taxonomy_series.iloc[
				0]  # Get first lineage _sample for notation testing assuming the rest have the the same notations
		...
		if notation == 'greengenes':
			...
			taxonomy = pd.DataFrame(index=ordered_indices_list, data=ordered_taxa_list,
	...
	def __init_taxonomy_from_frame(self,
								   taxonomy_dataframe: pd.DataFrame,
								   taxonomy_notation: Optional[str],
								   order_ranks: Optional[Sequence[
									   str]]) -> pd.DataFrame:  # Done # For now only pass to _init_taxonomy_from_series
		"""Main method that produces taxonomy dataframe from dataframe

		Args:
			taxonomy_dataframe: :class:`~pandas.DataFrame` with taxa split by ranks.
			taxonomy_notation: Taxonomy lineage notation style. Can be one of :const:`pmaf.internals._constants.AVAIL_TAXONOMY_NOTATIONS`
			order_ranks:  List with the target rank order. Default is set to None. The 'silva' notation require `order_ranks`.
		...
		if valid_ranks is not None:
			if len(valid_ranks) > 0:
				return pd.concat([taxonomy_dataframe, pd.DataFrame(data='', index=taxonomy_dataframe.index,
																   columns=[rank for rank in VALID_RANKS if
																			rank not in valid_ranks])], axis=1)
			else:
