...
import numpy as np
import biom
from typing import Union, Sequence, Mapping, TypeVar, Tuple, Callable, Any, Optional
from pmaf.internal._typing import GenericIdentifier, Mapper

T = TypeVar('T', bound='FrequencyTable')
...
class FrequencyTable(EssentialBackboneBase, EssentialFeatureMetabase, EssentialSampleMetabase):
		...
		if len(tmp_dtypes) == 1 and pd.api.types.is_numeric_dtype(tmp_dtypes[0]):
			...
			if allow_nan:
				if len(tmp_dtypes) == 1 and pd.api.types.is_numeric_dtype(tmp_dtypes[0]):
					...
					if all([(dt == object) or (pd.api.types.is_numeric_dtype(dt)) for dt in
							tmp_dtypes]) and tmp_frequency.isnull().values.any():
						self.__init_frequency_table(tmp_frequency)
					else:
	...
	def _remove_features_by_id(self,
							   ...
							   **kwargs) -> Union[None, GenericIdentifier, dict]:
	...
	def drop_features_by_id(self,
							ids: GenericIdentifier) -> Union[None, np.ndarray]:
	...
	def drop_samples_by_id(self,
						   ids: GenericIdentifier) -> Optional[np.ndarray]:
		"""Drop samples by `ids`.
	...
	def merge_features_by_map(self,
		...
		if isinstance(mapping, (dict, pd.Series)):
			tmp_ids = sorted({x for _, v in mapping.items() for x in
							  v})  # FIXME: Uncool behavior make it better and follow the usage.
	...
	def get_subset(self,
		...
		if not ((self.xrid.isin(target_rids).sum() == len(target_rids)) and (
				self.xsid.isin(target_sids).sum() == len(target_sids))):
			raise ValueError('Invalid ids are provided.')
		return type(self)(frequency=self.__internal_frequency.loc[target_rids, target_sids], metadata=self.metadata,
						  name=self.name)
