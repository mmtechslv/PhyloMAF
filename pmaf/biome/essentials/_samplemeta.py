...
class SampleMetadata(EssentialBackboneBase, EssentialSampleMetabase):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.__internal_samples = pd.concat(
            [
                tmp_sample.select_dtypes([], ["object"]),
                tmp_sample.select_dtypes(["object"]).apply(
    ...
    def merge_samples_by_variable(
        ...
        **kwargs
        ...
        return self._merge_samples_by_map(
    ...
    def get_subset(self, sids: GenericIdentifier = None, *args, **kwargs) -> 'SampleMetadata':
        ...
        if not self.xsid.isin(target_sids).sum() == len(target_sids):
            raise ValueError("Invalid sample ids are provided.")
        return type(self)(
            samples=self.__internal_samples.loc[target_sids, :],
            axis=0,
            metadata=self.metadata,
            name=self.name,
        )

    def _export(self, *args, **kwargs) -> Tuple[pd.DataFrame, dict]:
