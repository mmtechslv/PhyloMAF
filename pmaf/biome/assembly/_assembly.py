...
import pandas as pd
from os import path
from typing import Union, Sequence, TypeVar, Any, Optional
from pmaf.internal._typing import GenericIdentifier

T = TypeVar('T', bound='BiomeAssembly')
...
class BiomeAssembly(BiomeBackboneBase, BiomeAssemblyBackboneMetabase):
	...
	def __init__(self,
		...
		Args:
			...
			*args: Instances of `essentials`
			curb: Set limiting instance of `essentials` that will be used to subset remaining essentials to identical axes. If set tp 'intersect' intersection will be used as limiter.
		...
		for essential in tmp_essentials:
			if tmp_controller.verify_essential(essential, check_axis=curb is None, check_mount=not copy):
				if curb is not None:
					if isinstance(essential, EssentialFeatureMetabase) and not isinstance(essential,
																						  EssentialSampleMetabase):
						tmp_essentials_adj.append(essential.get_subset(tmp_target_rids))
					elif isinstance(essential, EssentialSampleMetabase) and not isinstance(essential,
	...
	def __getattr__(self,
					attribute: str) -> EssentialBackboneMetabase:
	...
	def __make_otu_table(self,
		...
		if not ((self.xrid.isin(target_rids).sum() == len(target_rids)) and (
				self.xsid.isin(target_sids).sum() == len(target_sids))):
			raise ValueError('Invalid ids are provided.')
		tmp_freq = self.__controller.take_essential_by_type(FrequencyTable).get_subset(rids=target_rids,
																					   sids=target_sids, **kwargs)
